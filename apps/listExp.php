...
EOPAGE;
	...
	$btn_options = '<button rel="{id}" class="btn btn-mini view" id="btn-view" title="Ver detalles"><i class="fa fa-eye"></i></button>';
} 
...
//$iten = file_get_contents('http://ifsmexico.com/iten/json/listIten.php?expimp=exp&m=' . $m . '&f='. $f . '&s=' . $s);
//$iten = json_decode($iten);

$iten = new Iten ();
$search = $iten->search($s);
$listr = $iten->listr('exp', $m, $f);
     ...
     $('.view').on('click',function(e){
		...
		<table id="example2" class="table table-striped table-bordered">
			<thead>
                <tr>
                    <th></th>
                    <th style="padding-right: 25px;">Destino</th>
                    <th style="padding-right: 25px;">Puerto de Carga</th>
                    ...
                    <th style="padding-right: 25px;">Cierre Desp.</th>
                    <th style="padding-right: 25px;">Frecuencia</th>
                    <th style="padding-right: 25px;">Tiempo de Transito</th>
...
<?php
	for ($i = 0; $i < count($listr); $i++){
		...
		$tmp = str_replace('{cierredesp}', $t['cierredesp'], $tmp);
