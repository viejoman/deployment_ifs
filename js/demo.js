...
var urlJsonTC = 'php/obtenerTipoCambio.php';

$(function () {

	//$('#anuncioModalIFS').modal('show');
	
    $("#widgetclima").change(function () {
        var idclima = this.value;
        $.post(
		    'apps/widgetclima.php',
		    {
		        'widgetclima': idclima
		    },
		    function (result) {
		        $('#widgeclima-body').html(result);
		    }
		);
    });
    ...
    $.getJSON(urlJsonTC,
	        function (res) {
                ...
                var fecha_tc = res.fecha_tc.split('-');

		        $("#fecha-tipo-cambio").html("Fecha: " + fecha_tc[2] + "/" + fecha_tc[1] + "/" + fecha_tc[0]);
                //$("#fecha-tipo-cambio").html("Fecha: " + res.fecha_tc);
		        $("#tipo-cambio").html(res.tc + " MXP");
	        }
	);    
    
    $("#cmbOficina").change(function () {
    	listDirectorio(this.value);
    });  
    
    listDirectorio = function (p_idOficina){
    	var idOficina = p_idOficina;
        $.post(
		    'apps/view_directorio.php',
		    {
		        'idOficina': idOficina
		    },
		    function (result) {
		        $('#directorio-body').html(result);
		    }
		);       
    ...
    demo_settings.append(
          "<h4 class='text-light-blue' style='margin: 0 0 5px 0; border-bottom: 1px solid #ddd; padding-bottom: 15px;'>Idioma</h4>"
    //Boxed layout
    ...
    */
    demo.click(function () {
        if (!$(this).hasClass("open")) {
            ...
            demo_settings.animate({ "right": "-100px" });
